...
- 2026-06: **Auto-sync scheduler fix** — new leads weren't importing until manual "Sync Now" because auto-sync relied solely on the external `.emergent/crons.yml` cron (unreliable in prod). Added an in-process FastAPI background loop (`periodic_sync_loop`, `SYNC_INTERVAL_MINUTES` env, default 5, set to 2) that runs `sync_leads_from_sheet()` on an interval regardless of external cron. Verified: deleting a sheet-sourced lead auto-reappears with no manual trigger.
